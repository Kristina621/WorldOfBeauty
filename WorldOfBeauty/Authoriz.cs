        ...
        public static void Auth(string Login, string Password)
        {
            ...
            try
            {
                ...
                if (LoginCount > 0)
                {
                    ...
                    SqlCommand CheckPasswordCmd = new SqlCommand(@"SELECT Count(*) FROM [User] WHERE Login=@Login and Password=@Password COLLATE Cyrillic_General_CS_AS", DBConnect.sql);
                    CheckPasswordCmd.Parameters.AddWithValue("@Login", Login);
                    string a = Crypt.GetHash(Password);
