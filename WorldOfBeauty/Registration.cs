...
namespace WorldOfBeauty
{
    class Registration
    {
        ...
        {
            if (login == "" || pass == "" || checkpass == "" || SurnClient == "" || NameClient == "") // если одно из поле пустое
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        try
                        {
                            if (MobileNum == string.Empty)
                                MobileNum = "+7(000)000-00-00";
                            logchk = 0;
                            DBActions.User_Insert(login, Crypt.GetHash(pass), 1); // добавляет зарегестрированные данные в бд 
                            ...
                            if (logchk != 0)
                            {
                                DBActions.client_Insert(SurnClient, NameClient, MidnClient, MobileNum);
